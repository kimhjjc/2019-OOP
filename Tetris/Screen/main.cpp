...
class GameMap {
	...
	int score;
	int scoreUITimer;
	Position scoreUIPos1;
	Position scoreUIPos2;
	bool tenScoreCheck;
	bool hundScoreCheck;
	int gameTime;
...
public:
	...
	GameMap(int width = 10, int height = 20)
		:width(width), height(height), nextWidth(NULL), nextHeight(NULL), breakBlockCount(NULL),
		score(NULL), scoreUITimer(NULL), scoreUIPos1(NULL), scoreUIPos2(NULL),
		tenScoreCheck(false), hundScoreCheck(false), gameTime(NULL),
		map(nullptr), drawMap(nullptr), nextBlock(nullptr), lineCheck(false), life(true),
	...
	void UIDraw() {
		...
		string UI3{ "Play Time        : " + to_string(gameTime = (clock() / 1000)) + " sec" };
		screen.draw("Next Block", 10, 1, Position(21, 1));
		screen.draw(UI1.c_str(), UI1.size(), 1, Position(20, 9));
		screen.draw(UI2.c_str(), UI2.size(), 1, Position(20, 10));
		screen.draw(UI3.c_str(), UI3.size(), 1, Position(20, 11));
		if (scoreUITimer > 0)
		{
			scoreUITimer--;
			if ((scoreUITimer % 4) == 0 && scoreUITimer > 4)
			{
				scoreUIPos1.x = 13 + rand() % 3; scoreUIPos1.y = 19 - rand() % 16;
				scoreUIPos2.x = 13 + rand() % 3; scoreUIPos2.y = 19 - rand() % 16;
			}
			if (tenScoreCheck) screen.draw("+10", 3, 1, scoreUIPos1);
			if (hundScoreCheck) screen.draw("+100", 4, 1, scoreUIPos2);
		}
		else { tenScoreCheck = false; hundScoreCheck = false; }
	}

	int& getScore() { return score; }
	void setScoreUITimer() { scoreUITimer = 17; tenScoreCheck = true; }
...
public:
	...
	void update() {
		...
		if (timer <= 0)
		{
			if (underCheck) getPos().y = (getPos().y + 1) % map.getHeight();
			else
			{
